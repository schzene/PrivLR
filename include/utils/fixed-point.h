#ifndef PRIV_LR_FIXED_POINT_H_
#define PRIV_LR_FIXED_POINT_H_
...
public:
    ...
    FixArray LUT(const vector<uint64_t> &spec_vec, const FixArray &x,
                 bool signed_, int l_out, int s_out, int l_in);
    // same as above LUT function except l_in is same as x.ell
    inline FixArray LUT(const vector<uint64_t> &spec_vec, const FixArray &x,
    ...
    vector<FixArray> digit_decomposition(const FixArray &x, int digit_size);
    ...
    vector<FixArray> msnzb_one_hot(const FixArray &x, int ell);

    // Finds max element in x[i], forall i
    FixArray max(const vector<FixArray> &x);

    FixArray max_iron(const vector<FixArray> &x);
