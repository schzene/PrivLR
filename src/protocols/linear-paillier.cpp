...
//         CipherVector in_a_secret_a(in_a_fixed, pk), in_b_secret_a(in_b_fixed, pk), res_part1_secret_a,
//             res_part2_secret_a;
...
double Linear::dot_product(const vector<double>& in_a, const vector<double>& in_b) const {
    ...
    if (party == ALICE) {
        ...
        vector<ZZ> res_part1_ring(size), res_part2_ring(size);
        for (size_t i = 0; i < size; i++) {
...
//             in_a_flatten_fixed[i * size + j]  = ZZ(uint64_t(
//                 (in_a_flatten_signed[i * size + j] ? -in_a_flatten[i * size + j] : in_a_flatten[i * size + j]) *
//                 (1ull << BIT_LENGTH)));
//             in_b_flatten_fixed[i * size + j]  = ZZ(uint64_t(
//                 (in_b_signed[j] ? -in_b_flatten[i * size + j] : in_b_flatten[i * size + j]) * (1ull << BIT_LENGTH)));
//         }
//     }

//     if (party == ALICE) {
//         vector<char> in_a_b_flatten_signed(data_size * size), in_b_b_signed(size);
//         CipherVector in_a_flatten_secret_a(in_a_flatten_fixed, pk), in_b_flatten_secret_a(in_b_flatten_fixed, pk),
//             res_part1_secret_a, res_part2_secret_a;
//         // send in_a_secret_a, in_b_secret_b to bob;
...
vector<double> Linear::dot_product(const vector<vector<double>>& in_a, const vector<double>& in_b,
    ...
    vector<double> res(data_size), in_b_flatten(data_size * size);
    vector<ZZ> in_a_flatten_ring(data_size * size), in_b_flatten_ring(data_size * size);
    for (size_t i = 0; i < data_size; i++) {
        for (size_t j = 0; j < size; j++) {
            res[i] += in_a_flatten[i * size + j] * in_b[j];
            in_b_flatten[i * size + j] = in_b[j];
            ...
            in_b_flatten_ring[i * size + j] =
                ZZ(static_cast<uint64_t>(in_b_flatten[i * size + j] * (1ULL << SCALE)) % (1ULL << BIT_LENGTH));
        }
    }
    ...
    if (party == ALICE) {
        ...
        for (size_t i = 0; i < data_size; i++) {
            for (size_t j = 0; j < size; j++) {
                ...
                res_part1[i * size + j] =
                    NTL::conv<int64_t>((res_part1_ring[i * size + j] >= ((1ULL << BIT_LENGTH) / 2)) ?
                                           (res_part1_ring[i * size + j] - ((1ULL << BIT_LENGTH))) :
                                           res_part1_ring[i * size + j]);
                res_part2[i * size + j] =
                    NTL::conv<int64_t>((res_part2_ring[i * size + j] >= ((1ULL << BIT_LENGTH) / 2)) ?
